...
export const applyFlowDrag = (orderedFlow, dragResult) => {
    ...
    if (dragResult.payload.start) {
        let endIndex = orderedFlow.indexOf(orderedFlow.filter(e => e.start == false && e.startInd == dragResult.payload.startInd)[0])
        let endValue = store.getters.randomSections.filter(e => e.startWith == dragResult.payload.startInd)[0].endWith
        ...
        if (dragResult.addedIndex < (endIndex - 1)) {
            ...
            for (let i = 0; i < store.getters.randomSections.length; i++) {
                if (store.getters.randomSections[i].startWith != dragResult.payload.startInd) {
                    if ((newValue >= store.getters.randomSections[i].startWith &&
                            newValue <= store.getters.randomSections[i].endWith) || // Inside the section
                        (newValue <= store.getters.randomSections[i].startWith &&
                            endValue >= store.getters.randomSections[i].endWith)) { // Encapsulating the section
                        return;
        ...
        let startIndex = orderedFlow.indexOf(orderedFlow.filter(e => e.start == true && e.startInd == dragResult.payload.startInd)[0])
        let startValue = store.getters.randomSections.filter(e => e.startWith == dragResult.payload.startInd)[0].startWith
                ...
                if (store.getters.randomSections[i].startWith != dragResult.payload.startInd) {
                    if ((newValue >= store.getters.randomSections[i].startWith &&
                        ...
                        (newValue >= store.getters.randomSections[i].endWith &&
                            startValue <= store.getters.randomSections[i].startWith)) { // Encapsulating the section
                        return;
        ...
        if (newOrder == null) {
            ...
            if (orderedFlow[dragResult.addedIndex].start) {
                ...
                if (dragResult.addedIndex > dragResult.removedIndex) {
                    ...
                    newOrder = store.getters.randomSections.filter(e => e.startWith == orderedFlow[dragResult.addedIndex].startInd)[0].endWith + 1
                }
        ...
        let thisBlock = store.getters.sortedBlocks[orderedFlow[dragResult.removedIndex].order - 1]

        let newOrderedFlow = orderedFlow
        // If dragging down
        if (dragResult.addedIndex > dragResult.removedIndex) {
            newOrderedFlow.splice(dragResult.addedIndex + 1, 0, orderedFlow[dragResult.removedIndex]);
            newOrderedFlow.splice(dragResult.removedIndex, 1);

        // If dragging up
        } else {
            newOrderedFlow.splice(dragResult.addedIndex, 0, orderedFlow[dragResult.removedIndex]);
        ...
        for (let i = 0; i < newOrderedFlow.length; i++) {
            if (newOrderedFlow[i].order != null) {
                ...
                let thisSection = store.getters.randomSections.filter(e => e.startWith == newOrderedFlow[i].startInd)[0]
                if (newOrderedFlow[i].start) {
                    // Also update the endWith coming to this startInd
                    newOrderedFlow.filter(e => !e.start && e.startInd == newOrderedFlow[i].startInd)[0].startInd = order + 1
        ...
        for (let i = store.getters.randomSections.length - 1; i > -1; i--) {
            if (store.getters.randomSections[i].endWith < store.getters.randomSections[i].startWith) {
                store.commit('removeRandomSection', store.getters.randomSections[i].startWith)
            }
        }
