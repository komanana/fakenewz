...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    async handleQuestionType(state, { oldquestion, type }) {
      ...
      const block_id = oldquestion.block || state.survey.editorData.blocks.find(i => i.questionData.questions.find(j => j.id === oldquestion.id)).id;
    ...
    async insertNewQuestion(state, { type, block }) {
      ...
      switch (type) {
        ...
        case 'Multiple choice':
          ...
          const rest1 = await SurveyServices.postMultiChoiceQuestion(newQuestion.typedata.id,
              {order: 1, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')})

          const rest2 = await SurveyServices.postMultiChoiceQuestion(newQuestion.typedata.id,
              {order: 2, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')})
          const rest3 = await SurveyServices.postMultiChoiceQuestion(newQuestion.typedata.id,
              {order: 3, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')})
          console.log(rest1)
          Vue.set(newQuestion, "choices", [{id: rest1.id, order: 1, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')},
            {id: rest2.id, order: 2, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')},
            {id: rest3.id, order: 3, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')}])
          break
        case 'Matrix table':
          newQuestion = {
            ...question,
        ...
        case 'Rank order':
          ...
          const ro1 = await SurveyServices.postRankOrderQuestion(newQuestion.typedata.id,
              {order: 1, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')})

          const ro2 = await SurveyServices.postRankOrderQuestion(newQuestion.typedata.id,
              {order: 2, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')})
          const ro3 = await SurveyServices.postRankOrderQuestion(newQuestion.typedata.id,
              {order: 3, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')})
          console.log(ro1)
          Vue.set(newQuestion, "choices", [{id: ro1.id, order: 1, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')},
            {id: ro2.id, order: 2, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')},
            {id: ro3.id, order: 3, question: question.id, text: i18n.t('app.MCDefault'), title: i18n.t('app.MCDefault')}])
          break
        case 'Number scale':
  ...
  actions: {
    ...
    async addComment({ commit }, val) {
      // console.log(val)
      const res = await SurveyServices.postComment(val.questionId, {content: val.content})
      // console.log(res)
