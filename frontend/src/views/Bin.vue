...
<template>
  ...
  <h1>{{ $t("g5.ms-bin") }}</h1>
    ...
    <table id="survey-table">
      ...
      <tr>
        <th><el-link :underline="false">
          {{ $t("home.card3Info1") }}
          <i class="el-icon-arrow-down"></i>
        </el-link></th>
        <th v-if="ifTitleOrder"><el-link :underline="false" @click="changeTitleOrder" :type="types[2]">
          {{ $t("g5.ms-title") }}
          <i class="el-icon-arrow-down"></i>
        </el-link></th>
        <th v-if="!ifTitleOrder"><el-link :underline="false" @click="changeTitleOrder" :type="types[3]">
          {{ $t("g5.ms-title") }}
          <i class="el-icon-arrow-up"></i>
        </el-link></th>
        <th>{{ $t("g5.Survey Language") }}</th>
        <th v-if="ifTimeOrder"><el-link :underline="false" @click="changeTimeOrder" :type="types[0]">
          {{ $t("g5.Create time") }}
          <i class="el-icon-arrow-down"></i>
        </el-link></th>
        <th v-if="!ifTimeOrder"><el-link :underline="false" @click="changeTimeOrder" :type="types[1]">
          {{ $t("g5.Create time") }}
          <i class="el-icon-arrow-up"></i>
        </el-link></th>
      ...
      <tr v-for="survey in showSurveys" :key="survey.id">
        <td><el-checkbox v-model="survey.checked" @change="changeStatus"></el-checkbox></td>
        <td>{{survey.name}}</td>
        ...
        <td>{{computeRespondents(survey.current_submission, survey.required_submission, survey.status)}}</td>
        <td>
          <span v-if="survey.status === 0"><i class="fas fa-circle" style="color:#1947e5"></i> {{ $t("g5.Saved") }} </span>
          <span v-if="survey.status === 1"><i class="fas fa-circle" style="color:#18e200"></i> {{ $t("g5.Published") }} </span>
          <span v-if="survey.status === 2"><i class="fas fa-circle" style="color:grey"></i> {{ $t("g5.Finished") }} </span>
        </td>
...
<!--          <a :href="getConfigurationExportUrl(survey.id)" class="action-btn">{{ $t("g5.ms-export-conf") }}</a>-->
          ...
          <a class="action-btn" @click="recoverSurvey(survey)">{{ $t("g5.ms-recover") }}</a>
        ...
        <el-dialog
          ...
          <el-form ref="form" label-width="20px" class="form-side">
            <el-form-item>
              <el-row>
                <el-col :span="18" >
                  Please select whether to allow the same user to answer multiple times?
                </el-col>
          ...
          <span slot="footer" class="dialog-footer">
            <el-button type="primary" @click="previousPublish">Previous</el-button>
            <el-button type="success" @click="finishSurvey(currentId)">Publish</el-button>
          </span>
        </el-dialog>
        <el-dialog
            :title= DialogTitle
          ...
          <el-row>
            ...
            <el-col :span="2">
              <el-button type="info" v-clipboard:copy = "link" v-clipboard:success="onCopy" v-clipboard:error="onError">CopyLink</el-button>
            </el-col>
          </el-row>
          <span slot="footer" class="dialog-footer">
            <el-button type="primary" @click="linkDialogVisible = false">Finish</el-button>
          </span>
        </el-dialog>
      </tr>
    </table>
...
export default {
  ...
  methods: {
    ...
    batchRecover() {
      // 获取已被选中的数据，为promises，是一个数组
      const promises = this.showSurveys.filter(i => i.checked)
      // 选中数量大于1
      if(promises.length > 1) {
        // 旧代码
        // this.showSurveys.filter(i => i.checked).forEach(i => {
        //   const promise = SurveyServices.patchSurvey(i.id, {deleted: 0})
        //   promises.push(promise)
        // });
        this.recoverSurvey(promises[0])
        promises.forEach(i => {
          const promise = SurveyServices.patchSurvey(i.id, {deleted: 0})
          promises.push(promise)
        });
    ...
    mulRecover() {
      ...
      if(list.length > 1) {
         this.$confirm(
          'Are you sure you want to recover the following survey?',
          'Reminder',
          {
    ...
    async finishSurvey(id){
      ...
      await SurveyServices.patchSurvey(id, {status: 1, publish_time: nowDate, is_repeat_answer: this.ifMultiple, if_capture_gaze: this.ifCaptureGaze})
      if(this.ifTime){
      ...
      if(this.ifVolume){
        await SurveyServices.patchSurvey(id, {required_submission: this.respondents})
      }
      if(this.ifDuration){
    ...
    showDurationExpireTime(){
      this.durationTime = 1000*60*60 * this.durationHours + 1000*60 * this.durationMinutes
    ...
    showExpireTime(){
      ...
      this.expireTime = new Date();
      this.expireTime.setTime(nowDate.getTime() + 1000*60*60*24 * this.days + 1000*60*60 * this.hours + 1000*60 * this.minutes)
      this.expireTimeString = this.expireTime.getFullYear() + "-" + (this.expireTime.getUTCMonth()+1) + "-" + this.expireTime.getUTCDate()+
          " " + this.expireTime.getHours() + ":" + this.expireTime.getMinutes()
    },
    async endSurvey(id) {
      this.$confirm(this.$t("bin.endSurveyReminder"), this.$t('g5.reminder'), {
          confirmButtonText: this.$t('g5.Confirm'),
          cancelButtonText: this.$t('g5.Cancel'),
        type: 'warning'
      }).then(async () => {
    ...
    async recoverSurvey(item) {
      this.$confirm(this.$t('bin.recoverReminder') + item.name, this.$t('g5.reminder'), {
        confirmButtonText: this.$t('g5.Confirm'),
          cancelButtonText: this.$t('g5.Cancel'),
        type: 'warning'
      }).then(async () => {
        this.$message({
          type: 'success',
          message: this.$t('bin.recoverSuccess')
        });
        await SurveyServices.patchSurvey(id, {deleted: 0})
        let index = this.surveys.indexOf(this.surveys.filter(e => e.id === id)[0])
        this.surveys.splice(index, 1)
    ...
    async deleteSurvey(id, name) {
      this.$confirm(this.$t('g5.deleteReminder') + name + ' ?', this.$t('g5.reminder'), {
        confirmButtonText: this.$t('g5.Confirm'),
        cancelButtonText: this.$t('g5.Cancel'),
        type: 'warning'
      }).then(async () => {
    ...
    async getSurveys() {
      await this.$axios.get("/api/surveys/deleted/" + this.userId).then(
          res =>{
    ...
    async getResponseExportUrl(id) {
      const response = await axios.get(this.$axios.defaults.baseURL + 'survey/responseNumber/' + id)
          .then(res =>{
            window.location.href = this.$axios.defaults.baseURL + 'survey/exportCsv/' + id
          })
          .catch(
          error => {
  ...
  async beforeMount() {
    await this.$axios.get('account/user/', {
      headers: {
