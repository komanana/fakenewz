<template>
  <div>
    <div>
      <button class="statistic-btn" v-on:click="getStatisticView()">Back</button>
      <div class="mainBlock">
        <div class="Block">
          <h2 class="timeName">{{ $t("surveyStatistic.timeInfo") }}</h2>
          <div class="metadata-display">
            <h3>{{ $t("surveyStatistic.minTime") }}</h3>
            <span class="metadata-figure">{{ statistic.timeData.minTime }}</span>
          </div>
          <div class="metadata-display">
            <h3>{{ $t("surveyStatistic.midTime") }}</h3>
            <span class="metadata-figure">{{ statistic.timeData.midTime }}</span>
          </div>
          <div class="metadata-display">
            <h3>{{ $t("surveyStatistic.maxTime") }}</h3>
            <span class="metadata-figure">{{ statistic.timeData.maxTime }}</span>
          </div>
          <div class="metadata-display">
            <h3>{{ $t("surveyStatistic.averageTime") }}</h3>
            <span class="metadata-figure">{{ statistic.timeData.averageTime }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="block-segment" v-for="(block,index) in this.statistic.blocks">
...
export default {
  ...
  beforeMount() {
    ...
    this.$axios.get("api/st/survey/result/0/" + this.$route.params.id).then((res) => {
      this.statistic = res.data;
      this.statisticView = true;
      console.log("Start")
    }).catch((reason) => {
      this.$axios.get("api/st/survey/result/1/" + this.$route.params.id).then((res) => {
        this.statistic = res.data;
        this.statisticView = true;
        console.log("Start")
      }).catch((reason) => {
  ...
  computed:{
    uniformity(){
      ...
      this.statistic.blocks.forEach((b) => {
        b.questions && b.questions.forEach((q) => {
          if(!q.choices)return
          ...
          arr.push({
            ...
            blockId:b.id,
            qTitle: q.title,
            uniformity: Math.max(...choArr)/choArr.reduce((pre,cur,idx) => pre+cur)
          })
        })
      })
