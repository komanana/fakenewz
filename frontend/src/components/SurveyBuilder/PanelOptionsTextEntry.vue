<template>
  <div class="options">
    <div class="text-with-button" title="Select to Show Options of Validate Answer">
      <!--Validate Answer-->
    ...
    <div v-if="question.validate">
      ...
      <div v-if="question.ansType !== 'Date'">
        <line-base class="light"/>
        <h1 v-if="question.ansType === 'Text'" class="close-header">{{ $t("pOptionsTextEntry.minCharLength") }} </h1>
        ...
        />
        <line-base class="light"/>
        <h1 v-if="question.ansType === 'Text'" class="close-header">{{ $t("pOptionsTextEntry.maxCharLength") }}</h1>
          <h1 v-else-if="question.ansType === 'Integer'" class="close-header">{{
              $t("pOptionsTextEntry.intUpperBound")
            }}</h1>
          <h1 v-else-if="question.ansType === 'Decimal'" class="close-header">{{ $t("pOptionsTextEntry.decUpperBound") }}</h1>
          <text-box-num
...
export default {
  ...
  methods: {
    ...
    async toggleValidationQuestion(store) {
      ...
      await SurveyServices.patchTextAnswerType(this.question.id, {validate: this.question.validate})
    },
    async updateTextboxMinValue(store, flag, value)
    {
      ...
      await SurveyServices.patchTextAnswerType(this.question.id, {textboxMin: this.question.textboxMin})
    },
    async updateTextboxMaxValue(store, flag, value)
    {
      ...
      await SurveyServices.patchTextAnswerType(this.question.id, {textboxMax: this.question.textboxMax})
    }
  },
};
</script>
