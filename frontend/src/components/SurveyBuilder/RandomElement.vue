...
export default {
  ...
  computed: {
    ...
    showing() {
      let text = ''
      if (this.start != null) {
        let thisSection = this.randomSections.filter(e => e.startWith == this.start)[0]
        text = ` (show ${thisSection.display} out of ${thisSection.endWith - thisSection.startWith + 1})`
      }
