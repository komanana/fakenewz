      ...
      >
        ...
        <div v-else class="block">
          <div class="block-header">
            ...
            <div class="button-row">
              <button-base
                  ...
                  @buttonPress="addRandomSection(orderedFlow[index].order, orderedFlow[index].survey)"
                  :hover="'Random'"
              />
              <button-base
          ...
          <h3
              v-if="
                showingQuestions[orderedFlow[index].order - 1] &&
                sortedBlocks[orderedFlow[index].order - 1].questionData
                  .questions.length == 0
              "
          >
...
export default {
  ...
  methods: {
    ...
    onDrop(dropResult) {
      ...
      let thisShowing = this.showingQuestions[this.orderedFlow[dropResult.removedIndex].order - 1]
      this.showingQuestions.splice(this.orderedFlow[dropResult.removedIndex].order - 1, 1)

      // If moving down
      if (dropResult.addedIndex > dropResult.removedIndex) {
        this.showingQuestions.splice(this.orderedFlow[dropResult.addedIndex].order - 1, 0, thisShowing)
