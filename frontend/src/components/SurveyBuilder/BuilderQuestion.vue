  ...
  >
    ...
    <div v-if="question.type === 'Text entry'" class="question-content">
      <el-button type="primary" style="cursor:pointer"
                 :onclick="'document.getElementById(\'question_'+question.id+'\').click()'" title="Click to upload an image"
      >
        {{$t("builderBlock.uploadImage")}}
      </el-button>
      <el-button type="info" style="cursor:pointer"
                 @click="cleanPhoto(question)" title="Click to clean the upload image"
      >
        {{$t("g5.cancel")}}
      </el-button>
      ...
      <div style="width: 100%; text-align: center">
        <br>
        <img :src="question.image" alt="" style="width:90%">
      </div>
      <br>
    ...
    <div v-if="question.type === 'Button row'" class="question-content">
      ...
      <h2 title="Add button rows in left pannel">{{ $t("builderQuestion.buttonContent") }}</h2>
      <div
      ...
      <h2>{{ $t("builderQuestion.JumpBlock") }}</h2>
        <el-select v-model="question.buttons[question.buttons.indexOf(button)].jumpBlockId" placeholder=""
                   @focus="getBlockList()" size="medium" filterable=true
                   @change="
            (input) =>
              inputTextboxButton(
                question.buttons[question.buttons.indexOf(button)]
    ...
    <div v-if="question.type === 'Multiple choice'" class="question-content">
      <el-button type="primary" style="cursor:pointer"
                 :onclick="'document.getElementById(\'question_'+question.id+'\').click()'" title="Click to clean the upload image"
      >
        {{$t("builderBlock.uploadImage")}}
      </el-button>
      <el-button type="info" style="cursor:pointer"
      ...
      <h2 title="Default unallow multiple answer">{{ $t("builderQuestion.multipleChoiceOptions") }}</h2>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <!-- {{matrixChecked[scope.row.name + column.label]}} -->
              <el-checkbox :key="matrixChecked[scope.row.name + column.label]" :checked="matrixChecked[scope.row.name + column.label]" @change="handleMatrixChange(scope.row.name, column.label, $event)"></el-checkbox>
            </div>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="group-wrapper">
        ...
        <div class="group-right">
          <Container
            ...
            :get-child-payload="itemIndex => getChildPayloadChil(itemIndex,inx, 'chil')"
          >
    ...
    <div v-if="question.type === 'Rank order'" class="question-content">
      <h2 title="Input rank levels in each rows">{{ $t("builderQuestion.RankOrderOptions") }}</h2>
      {{choicesSort(question)}}
      <Container
    ...
    <div v-if="question.type === 'Drag and Drop'" class="question-content">
      <h2 title="This is for location">{{ $t("builderQuestion.DnDChips") }}</h2>
      <div
        v-for="choice in question.choices"
    ...
    <div v-if="question.type === 'Number scale'" class="question-content">
      ...
      <div v-if="question.minTitleOn">
        <h2 title="Set minimum value in left panel">{{ $t("builderQuestion.minTitle") }}</h2>
      ...
      <div v-if="question.midTitleOn">
        <h2 title="Set middle value in left panel">{{ $t("builderQuestion.midTitle") }}</h2>
      ...
      <div v-if="question.maxTitleOn">
        <h2 title="Set maximum value in left panel">{{ $t("builderQuestion.maxTitle") }}</h2>
      ...
      <h2 style="text-align: center" title="Preview what survey takers will see">
...
export default {
  ...
  methods: {
    ...
    getChildPayloadChil (itemIndex,inx,type) {
      return {...this.columnConfig[inx].children[itemIndex-1],type,delIdx:itemIndex-1}
    },
    groupsDrop(group,e){
      console.log(group,e)
      // if()
      if (e.payload.type === 'root') {
        ...
        this.columnConfig[this.columnConfig.findIndex(el => el.value === group.value)].children.splice(e.addedIndex-1,0,e.payload)
        ...
        if (e.removedIndex!== null) {
          this.columnConfig[this.columnConfig.findIndex(el => el.value === group.value)].children.splice(e.removedIndex-1,1)
        }
    ...
    choicesSort(questionData){
      ...
      for (let i = 1; i < questionData.choices.length; i++){
        for (let j = 0; j < questionData.choices.length - i; j++){
          if(questionData.choices[j].order > questionData.choices[j+1].order){
    ...
    async inputTextboxButton(button) {
      await SurveyServices.patchButtonRowQuestion(this.question.typedata.id, {
        ...
        jumpBlockId: this.question.buttons[this.question.buttons.indexOf(button)].jumpBlockId
      });
    },
  ...
  computed: {
    ...
    tableConfig: {
      get () {
        let data = this.question.typedata.tableConfig ? JSON.parse(this.question.typedata.tableConfig) : []
        data = data.filter((el) => !this.columnConfig.some((el2)=> el2.children && el2.children.some((el3)=> el3.name === el.name)))
        return data
      },
      set (val) {
