<template>
  <div class="question-box" :style="color">
    <div class="title">
      <h1>Q{{ number + 1 }}<span style="color: red" v-show="requiredShow">*</span></h1>
    ...
    <div v-if="showInput"  style="width: 100%; text-align: center">
      <el-input v-model="otherInputText" style="width: 97%;" @blur="sendResult" :placeholder="$t('textBox.yourAnswer')"></el-input>
    </div>
    <span class="tip-box"  v-if="oneOrMore===2"><span style="color: red">Tips:</span> Please select {{this.questionData.textboxMin}} ~ {{this.questionData.textboxMax}} choices!</span>
  </div>
  ...
  computed: {
    ...
    tableConfig: {
      get () {
        ...
        data = data.filter((el) => !this.columnConfig.some((el2)=> el2.children.some((el3)=> el3.name === el.name)))
        return data
      },
      set (val) {
  ...
  methods: {
    ...
    checkOneOrMoreRules() {
      if (this.questionData.multipleAnswers) {
        if (this.questionData.textboxMin === 1 && this.questionData.textboxMax === 1 ){
          this.oneOrMore = 1
        }else {
          this.oneOrMore = 2
        }
      ...
      if (this.questionData.required) {
        if (this.checked.length === 0) {
          ...
          if (this.questionData.multipleAnswers && this.oneOrMore === 2) {
            if (this.checked.length >= this.questionData.textboxMin
