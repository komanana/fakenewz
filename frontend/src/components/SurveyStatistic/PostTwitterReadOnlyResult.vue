<template>
  <div class="out-box">
    <div class="post" >
      <a :href="question.articleURL" v-if="isImage"><img :src="question.articleImage" :alt="question.articleTitle" width="100%" height="auto"/></a>

      <youtube v-else-if="this.questionData.articleStyle === 'Youtube'"
               :width="youtubeWidth"
               :height="youtubeHeight"
               :resize="true"
               class="youtube"
               :video-id="question.articleSource" ref="youtube"></youtube>
      <div v-else class="no-image">{{ $t("postEditable.noImage") }}</div>
      <h1><a :href="question.articleURL" style="text-decoration:none; color: black">{{ question.articleTitle }}</a></h1>
      <h2>
    ...
    <el-row style="margin: 10px;">
      ...
      <el-col :style="colWidth" v-show="replyShow">
        <div class="clickItem-reply"
             title="Reply">
          <i class="el-icon-chat-round clickItem-icon-reply" :style="iconColorReply" @click="clickItem('Reply')"></i>
          <span class="clickItem-title-reply" :style="textColorReply">{{ this.questionData.articleComments }}</span>
        </div>
      </el-col>

      <!--retweet-->
      <el-col :style="colWidth" v-show="retweetShow">
        <div class="clickItem-retweet"
             :title="retweetTitle">
          <i class="el-icon-share clickItem-icon-retweet" :style="iconColorRetweet"></i>
          <!--          <i class="el-icon-share clickItem-icon-retweet" :style="iconColorRetweet" @click="clickItem('Retweet')"></i>-->
          <span class="clickItem-title-retweet" :style="textColorRetweet">{{ this.questionData.articleShares }}</span>
        </div>
      </el-col>
      ...
      <el-col :style="colWidth" v-show="likeShow">
        <div class="clickItem-like"
             :title="likeTitle">
          <i class="el-icon-star-off clickItem-icon-like" :style="iconColorLike"></i>
          <!--          <i class="el-icon-star-off clickItem-icon-like" :style="iconColorLike" @click="clickItem('Like')"></i>-->
          <span class="clickItem-title-like" :style="textColorLike">{{ this.questionData.articleLikes }}</span>
        </div>
      </el-col>

      <!--add1-->
      <el-col :style="colWidth" v-show="addon1Show">
        <div class="clickItem-add1"
             :title="add1Title">
          <i class="clickItem-icon-add1" :class="add1Icon" :style="iconColorAdd1" @click="clickItem('Add1')"></i>
          <span class="clickItem-title-add1" :style="textColorAdd1">{{ add1Count }}</span>
        </div>
      </el-col>

      <!--add2-->
      <el-col :style="colWidth" v-show="addon2Show">
        <div class="clickItem-add2"
             :title="add2Title">
          <i class="clickItem-icon-add2" :class="add2Icon" :style="iconColorAdd2" @click="clickItem('Add2')"></i>
          <span class="clickItem-title-add2" :style="textColorAdd2">{{ add2Count }}</span>
        </div>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    changeReplyColor(flag) {
      if (flag === true) {
        this.iconColorReply = "font-size:15px; color: #479BE9; background: #E6EEF6;"
    ...
    changeRetweetColor(flag) {
      if (flag === true) {
        this.iconColorRetweet = "font-size:15px; color: #53B681;background: #E6F1EB;"
    ...
    changeLikeColor(flag) {
      if (flag === true) {
        this.iconColorLike = "font-size:15px; color: #E53B7F;background: #F5E4EB;"
    ...
    changeAdd1Color(flag) {
      if (flag === true) {
        this.iconColorAdd1 = "font-size:15px; color: #479BE9; background: #E6EEF6;"
    ...
    changeAdd2Color(flag) {
      if (flag === true) {
        this.iconColorAdd2 = "font-size:15px; color: #53B681;background: #E6F1EB;"
  ...
  computed: {
     ...
     */
    isImage() {
      console.log(this.questionData.articleImageLink)
      if (
          this.questionData.articleImageLink === "" ||
          this.questionData.articleImageLink == null
      ) {
        return false;
      }
