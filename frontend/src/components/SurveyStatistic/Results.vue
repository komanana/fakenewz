<template>
  <div class="mainBlock">
    ...
    <div v-for="(item,index) in blockData.questions" :key="blockData.questions.id">
      <MultipleChoiceResult v-if="item.type ==='Multiple choice'" :question="item" :index="index"/>
      <MatrixTableResult v-if="item.type ==='Matrix table'" :question="item" :index="index"/>
      <GroupsResult v-if="item.type ==='Groups'" :question="item" :index="index"/>
      <TextEntryResult v-if="item.type ==='Text entry'" :question="item" :index="index"/>
      <NumberScaleResult v-if="item.type ==='Number scale'" :question="item" :index="index"/>
      <ButtonRowResult v-if="item.type ==='Button row'" :question="item" :index="index"/>
      <NewsPostResult v-if="item.type ==='News post'" :question="item"  :index="index"/>
    </div>
  </div>
</template>
...
export default {
  ...
  name: "Results",
  components: {NewsPostResult, ButtonRowResult, MultipleChoiceResult, NumberScaleResult, TextEntryResult,MatrixTableResult,GroupsResult},
}
</script>
<style>
