<template>
  <div class="question-box" :style="color">
    <div class="title" style="margin-left: 10%">
      <h1 class="questionNumber" v-model="index">Q{{ index + 1 }} {{ questionData.type }}</h1>
      <h1 style="color: red" v-show="requiredShow">*</h1>
      <p style="font-size: large;font-weight: bold">{{ questionData.title }}</p>
    </div>
    <div class="newspost">
      <PostTwitterReadOnlyResult
          @updateAnswer="getData"
          @changeValue="changeValue"
          :question="questionData"></PostTwitterReadOnlyResult>
    </div>

    <div class="wordSelection">
      <el-carousel style="width: 400px;" height="200px" :interval="5000" arrow="always">
        <el-carousel-item v-for="item in this.questionData.comments"
                          style="background-color: transparent; text-align: center;display:table;word-break: break-word"
                          :key="item">
          <span style=" display:table-cell;vertical-align:middle;">{{ item }}</span>
        </el-carousel-item>
      </el-carousel>
    </div>
    <div class="random">
      <button class="action-btn" @click="randomSelection">{{$t("surveyStatistic.randomAnswers")}}</button>
      <button class="last-btn" @click="lastSelection">{{$t("surveyStatistic.lastFiveAnswers")}}</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async randomSelection() {
      let value = await SurveyServices.getRandomFiveResults(this.question.question_id);
    ...
    async lastSelection() {
      let value = await SurveyServices.getLastFiveResults(this.question.question_id);
      if (this.replyOn === true) {
        this.questionData.comments = value.comments;
      } else if (this.addOn1 === true) {
        this.questionData.comments = value.addon1;
    ...
    async changeValue(value, flag) {
      let val = await SurveyServices.getLastFiveResults(this.question.question_id);
      if (flag === 1) {
        this.replyOn = true;
        this.addOn1 = false;
        this.addOn2 = false;
    ...
    checkValidity() {
      if (this.questionData.required === true) {
        let flag = false
        for (let i = 0; i < this.resultObj.response_question_answer.length; i++) {
