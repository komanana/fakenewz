  ...
  <div>
    <div class="question-box">
      <div class="word">
        ...
        <h2 class="questionName" v-model="questionData">{{ questionData.title }}</h2>
      </div>
      <el-carousel :autoplay="false" indicator-position="outside">
        <el-carousel-item style="background-color:#fff;" :key="'chart'">
          <div id="chart">
            <apexchart height="300px" type="bar" :options="chartOptions" :series="series"></apexchart>
          </div>
        </el-carousel-item>
        <el-carousel-item style="background-color:#fff;" v-for="el, i in tableDataList" :key="i">
          <el-table :data="el.data" style="width: 100%">
            <el-table-column prop="field" label="Field" width="200px">
            </el-table-column>
            
            <el-table-column v-for="el2,idx in parTableConfig" :prop="String(idx+1)" :label="String(idx+1)" :key="idx" :formatter="(r,c,cellValue)=> cellValue||'0'">
            </el-table-column>
            <el-table-column fixed="right" label="Total" width="100">
              <template slot-scope="scope">
                {{parTableConfig.reduce((pv,cv,id) => (scope.row[String(id+1)]||0)+pv,0)}}
              </template>
            </el-table-column>
...
export default {
  ...
  created() {
    ...
    const parAnswers = this.question.answers.map((e) => JSON.parse(e.replace(/\'/g, '"')))
